...
if('POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['register'])) {
	...
	if ($posted['voornaam'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een voornaam in.', 'neem'));
	if ($posted['achternaam'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een achternaam in.', 'neem'));
	if ($posted['adres'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een adres in.', 'neem'));
	if ($posted['huisnummer'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een huisnummer in.', 'neem'));
	if ($posted['postcode'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een postcode in.', 'neem'));
	if ($posted['geboortedatum'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een geboortedatum in.', 'neem'));
	if ($posted['telefoonnummer'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een telefoonnummer.', 'neem'));
	if ($posted['email'] == null)
		array_push($register_errors,  sprintf('<strong>Opmerking</strong>: Vul een email in.', 'neem'));
	if(emailExist($posted['email'])){
		array_push($register_errors, sprintf('<strong>Opmerking</strong>: De email bestaat al.', 'neem'));
	}
...
if('POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['login'])) {	
	...
	$mismatchErr = '';
	if ($password == null )
		array_push($login_error, sprintf('<strong>Opmerking</strong>: Vul een wachtwoord in.', 'neem'));
	if ($username == null )
		array_push($login_error, sprintf('<strong>Opmerking</strong>: Vul een geldig email in.', 'neem'));
	...
	if (empty($log_error)) {   //Check whether everything entered to create new user.
		...
		if($loginn){
			...
			$mismatchErr .=  sprintf('<p> <strong>Opmerking</strong>: Vul een geldig email in. </p>', 'neem');			
		}
	}
}

if('POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['adminlogin'])) {	
	$username = stripslashes(trim($_POST['username']));
	$password = stripslashes(trim($_POST['password']));
	$mismatchErr = '';
	if ($password == null )
		array_push($login_error, sprintf('<strong>Opmerking</strong>: Vul een wachtwoord in.', 'neem'));
	if ($username == null )
		array_push($login_error, sprintf('<strong>Opmerking</strong>: Vul een email in.', 'neem'));
	$log_error = array_filter($login_error);
	if (empty($log_error)) {   //Check whether everything entered to create new user.
		$loginn = login($username, $password);
		if($loginn){
			...
			$mismatchErr .=  sprintf('<p> <strong>Opmerking</strong>: Please enter Valid Credentials. </p>', 'neem');			
		}
	}
}
?>
...
<?php } elseif(isset($_GET['action']) && $_GET['action'] == 'verified') { ?>
    <hgroup>
      <h1>Registered</h1>
    </hgroup>
	<div style="background:white;">
	<span style="color:#000;line-height:2;">
        Dank u wel voor het verifiëren.
		U kunt zich nu inloggen door <a href="http://localhost/user/login.php">hier op te drukken</a>
		</span>
	  </div>
<?php } elseif(isset($_GET['action']) && $_GET['action'] == 'loginadmin') { ?>
	<hgroup>
      <h1>Admin Login</h1>
    </hgroup>
    <form action="" method="post" >
