...
FROM webshop.order o
	...
	WHERE o.fk_customer_id = '".$_SESSION['user_id']."' ";	
				$result = mysqli_query($db, $sql);
if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<td> " . $row["order_id"]."</a> </td><td> " . $row["order_time"]. " </td><td>  " . $row["paid"]. "</td><td>  " . $row["shipped"]. " </td><td> <a href='showorder.php?id=" . $row["order_id"]. "'>Zie order details</a></td></tr>";
    }
} else {
    echo "0 results";
}
